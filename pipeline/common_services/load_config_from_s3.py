...
from common.get_config_from_s3 import download_packages_from_s3, download_regions_from_s3, download_python_versions_from_s3
...
def main(event, context):
    ...
    # Load Packages
    response = load_config(python_version="all", config_type="pyVrsns", config_items=python_versions)
    logger.info({"message": "Loaded python_versions", "python_versions": python_versions, "response": response})
    ...
    for python_version in python_versions:
        ...
        response = load_config(python_version=python_version, config_type="pckgs", config_items=packages)
        logger.info({"message": "Loaded packages", "python_version": python_version, "packages": packages, "response": response})

        # Load regions
        response = load_config(python_version=python_version, config_type="rgns", config_items=regions)
        logger.info({"message": "Loaded regions", "python_version": python_version, "regions": regions, "response": response})
